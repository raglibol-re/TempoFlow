...
`POST /api/stripe/create-topup-checkout-session`. Credit is added after
`/api/stripe/webhook` verifies Stripe's signature and writes a confirmed
`stripe_topup` ledger transaction. Local development also syncs a returned
Checkout Session server-side after Stripe redirects back, so you can test without
the Stripe CLI running. Duplicate Stripe sessions/payment intents are ignored by
unique ledger indexes.
